		...
		if (not bar) then
			...
			if (self.Mode == "ICON") then
				...
				if (bar.count) then
					...
					bar.count:SetFont(C["media"].pxfont, 12, "OUTLINEMONOCHROME")
				...
				if (bar.statusbar) then
					...
					bar.statusbar:SetStatusBarTexture(C["media"].norm)
				...
				if (bar.background)then
					...
					bar.background:SetTexture(C["media"].norm)
				...
				if (bar.time)then
					...
					bar.time:SetFont(C["media"].pxfont, 14, "OUTLINEMONOCHROME")
