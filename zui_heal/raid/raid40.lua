local Z, C, L = unpack(Zodui) -- Import Functions/Constants, Config, Locales

local oUF = zUF or oUF
assert(oUF, "ZUI was unable to locate oUF.")

if not C["raidframes"].enable == true then return end
if IsAddOnLoaded("zui_dps") then return end

local RAID_WIDTH = ((ZuiBar1:GetWidth() / 5) - 2.5)*C["raidframes"].scale
...
local function Shared(self, unit)
  ...
  self:SetAttribute('initial-height', RAID_HEIGHT)
	...
	health.value:SetFont(C["media"].pxfont, (C["raidframes"].fontsize)*C["raidframes"].scale, "OUTLINEMONOCHROME")
	...
	RaidDebuffs.count = RaidDebuffs:FontString('count', C["media"].pxfont, C["raidframes"].fontsize*0.75, "OUTLINEMONOCHROME")
	RaidDebuffs.count:Point('BOTTOMRIGHT', RaidDebuffs, 'BOTTOMRIGHT', 0, 2)
	RaidDebuffs.count:SetTextColor(1, .9, 0)
	
	RaidDebuffs:FontString('time', C["media"].pxfont, C["raidframes"].fontsize, "OUTLINEMONOCHROME")
...
oUF:RegisterStyle('ZuiHealR26R40', Shared)
oUF:Factory(function(self)
	oUF:SetActiveStyle("ZuiHealR26R40")	
	local raid = self:SpawnHeader("ZuiHealR26R40", nil, "custom [@raid26,exists] show;hide",
	...
	raid:SetPoint("BOTTOM", ZuiBar1, "TOP", 0, Z.Scale(6))	
