...
local function SetChatStyle(frame)
	...
	_G[chat.."TabText"]:SetFont(C.media.pxfont, 12, "OUTLINEMONOCHROME")
	...
	_G[chat]:SetMinResize(C.general.infowidth-12,C.chat.height-10)
	
	-- move the chat edit box
	_G[chat.."EditBox"]:ClearAllPoints()
	_G[chat.."EditBox"]:Point("TOPLEFT", ZuiTabsLeftBackground or ZuiInfoLeft, 2, -2)
	_G[chat.."EditBox"]:Point("BOTTOMRIGHT", ZuiTabsLeftBackground or ZuiInfoLeft, -2, 2)	
	...
	local EditBoxBackground = CreateFrame("frame", "ZuiChatchatEditBoxBackground", _G[chat.."EditBox"])
	EditBoxBackground:CreatePanel("Default", 1, 1, "LEFT", _G[chat.."EditBox"], "LEFT", 0, 0)
	EditBoxBackground:ClearAllPoints()
	EditBoxBackground:SetAllPoints(ZuiTabsLeftBackground or ZuiInfoLeft)
...
local function SetupChatPosAndFont(self)	
	for i = 1, NUM_CHAT_WINDOWS do
		...
		if i == 1 then
			...
			chat:Point("BOTTOMLEFT", ZuiInfoLeft, "TOPLEFT", 4, 6)
			chat:Point("BOTTOMRIGHT", ZuiInfoLeft, "TOPRIGHT", -4, 6)
			--FCF_SavePositionAndDimensions(chat)
		elseif i == 4 then
			if not chat.isDocked then
				chat:ClearAllPoints()
				chat:SetFrameStrata("MEDIUM")
				chat:Point("BOTTOMRIGHT", ZuiInfoRight, "TOPRIGHT", -4, 6)
				chat:Point("BOTTOMLEFT", ZuiInfoRight, "TOPLEFT", 4, 6)
				FCF_SavePositionAndDimensions(chat)
			end
    else
      chat:SetFrameStrata("MEDIUM")
		...
		if C.chat.background and point == "BOTTOMRIGHT" and chat:IsShown() then
			ZuiChatBackgroundRight:Show()
			ZuiTabsRightBackground:Show()			
		end
	end
	...
	BNToastFrame:HookScript("OnShow", function(self)
		self:ClearAllPoints()
		if C.chat.background and ZuiChatBackgroundLeft then
			self:Point("BOTTOMLEFT", ZuiChatBackgroundLeft, "TOPLEFT", 0, 6)
		else
