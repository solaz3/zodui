...
local function Print (x)
	DEFAULT_CHAT_FRAME:AddMessage("|cffC495DDZui:|r " .. x)
end
...
function Stuffing:SlotUpdate(b)
	if not ZuiBags:IsShown() then return end -- don't do any slot update if bags are not show
...
function Stuffing:CreateBagFrame(w)
	...
	if w == "Bank" then
		f:Point("BOTTOMLEFT", ZuiInfoLeft, "TOPLEFT", 0, 2)
	else
		f:Point("BOTTOMRIGHT", ZuiInfoRight, "TOPRIGHT", 0, 2)
	end
	...
	f.b_close:SetScript("OnClick", function(self, btn)
		if self:GetParent():GetName() == "ZuiBags" and btn == "RightButton" then
...
function Stuffing:Layout(lb)
	...
	if lb then
		bs = bags_BANK
		if C.general.infowidth >= 405 then
			cols = 11
		elseif C.general.infowidth >= 370 and C.general.infowidth < 405 then
			cols = 10
		elseif C.general.infowidth >= 335 and C.general.infowidth < 370 then
			cols = 10
		else
			cols = 10
		end
		...
		f.editbox:SetFont(FONT, 8)
		f.detail:SetFont(FONT, 8)
		f.gold:SetFont(BAGSFONT, 12, "OUTLINEMONOCHROME")
...
function Stuffing:ADDON_LOADED(addon)
	...
	tinsert(UISpecialFrames,"ZuiBags")
...
function Stuffing:PLAYER_ENTERING_WORLD()
	...
	ContainerFrame1:Point("BOTTOMRIGHT", ZuiInfoRight, "TOPRIGHT", 4, 5)
	ContainerFrame1.ClearAllPoints = Z.dummy
	ContainerFrame1.SetPoint = Z.dummy
end
