	...
	for bar in self:GetBarIterator() do
		if not bar.injected then
				...
				if not texture.styled then
					texture:SetTexture(C["media"].norm)
					texture.styled=true
				end
				
				tbar:SetStatusBarTexture(C["media"].norm)
				...
				if not timer.styled then	
					...
					timer:SetFont(C["media"].pxfont, 12, "OUTLINEMONOCHROME")
...
local SkinBoss=function()
	...
	while (_G[format("DBM_BossHealth_Bar_%d", count)]) do
		...
		if not progress.styled then
			progress:SetStatusBarTexture(C["media"].norm)
...
local ForceOptions = function()
	...
	DBT_SavedOptions["DBM"].Texture = C["media"].norm
