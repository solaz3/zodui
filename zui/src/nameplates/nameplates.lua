local Z, C, L = unpack(select(2, ...)) -- Import Functions/Constants/Variables, Config, Locales

--Base code by Dawn (dNameplates), Rewritten by Elv22
if not C["nameplate"].enable then return end

local TEXTURE = C["media"].norm
local FONT = C["media"].font
local NFONT = C["media"].pxfont
local FONTSIZE = 12
local NFONTSIZE = 12
local FONTFLAG = "THINOUTLINE"
local NFONTFLAG =  "OUTLINEMONOCHROME"
local hpHeight = 12
local hpWidth = 110
...
local numChildren = -1
local frames = {}
local noscalemult = Z.mult * C["general"].uiscale

--Change defaults if we are showing health text or not
if not C["nameplate"].showhealth then
	hpHeight = 7
	iconSize = 20
end
...
if Z.isctm then
  ...
  if C["nameplate"].overlap or Z.eyefinity then
...
local function HideObjects(parent)
	for object in pairs(parent.queue) do
		if(object:GetObjectType() == 'Texture') then
			...
			object.SetTexture = Z.dummy
		elseif (object:GetObjectType() == 'FontString') then
			object.ClearAllPoints = Z.dummy
			object.SetFont = Z.dummy
			object.SetPoint = Z.dummy
			object:Hide()
			object.Show = Z.dummy
			object.SetText = Z.dummy
			object.SetShadowOffset = Z.dummy
		else
			object:Hide()
			object.Show = Z.dummy
		end
...
local function UpdateThreat(frame, elapsed)
	...
	if not C["nameplate"].enhancethreat then
		...
		if not frame.region:IsShown() then
			if InCombatLockdown() and frame.hasclass ~= true and frame.isFriendly ~= true then
				--No Threat
				if Z.isctm and Z.role == "Tank" then
	...
	if C["nameplate"].showhealth then
		frame.hp.value:SetText(Z.ShortValue(valueHealth).." - "..(string.format("%d%%", math.floor((valueHealth/maxHealth)*100))))
	end
	...
	elseif (frame.hasclass ~= true and frame.isFriendly ~= true) and C["nameplate"].enhancethreat then
		frame.healthborder_tex1:SetTexture(unpack(C["media"].bordercolor))
		frame.healthborder_tex2:SetTexture(unpack(C["media"].bordercolor))
		frame.healthborder_tex3:SetTexture(unpack(C["media"].bordercolor))
		frame.healthborder_tex4:SetTexture(unpack(C["media"].bordercolor))
...
local function SkinObjects(frame)
	...
	hp.level:SetFont(NFONT, NFONTSIZE, NFONTFLAG)
	hp.level:SetTextColor(1, 1, 1)
	hp.level:SetShadowOffset(Z.mult, -Z.mult)	
	...
	if C["nameplate"].showhealth then
		...
		hp.value:SetFont(NFONT, NFONTSIZE, NFONTFLAG)
		hp.value:SetPoint("CENTER", hp)
		hp.value:SetTextColor(1,1,1)
		hp.value:SetShadowOffset(Z.mult, -Z.mult)
	end
	...
	cb.time:SetFont(NFONT, NFONTSIZE, NFONTFLAG)
	cb.time:SetTextColor(1, 1, 1)
	cb.time:SetShadowOffset(Z.mult, -Z.mult)
	...
	cb.name:SetShadowOffset(Z.mult, -Z.mult)
	...
	name:SetShadowOffset(Z.mult, -Z.mult)
...
local function CheckBlacklist(frame, ...)
	if Z.PlateBlacklist[frame.name:GetText()] then
