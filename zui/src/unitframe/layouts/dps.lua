...
local function Shared(self, unit)
	...
	if unit == "player" then
		...
		if C["unitframes"].swing == true then
			local swing = Z.BuildSwingBar(self, CASTBAR_WIDTH - (BORDER*2), POWERBAR_HEIGHT - (BORDER*2), false)
		...
		--[[PvP Text
		self:FontString("PvP", NUMFONT, FONTSIZE, "OUTLINEMONOCHROME")
		...
		--Aggro Glow
		if C["unitframes"].displayaggro then
      ...
      reputation:Size(ZuiMinimap:GetWidth()-(BORDER*2), BORDER)
		...
		--Class Resource Bars
		if C["unitframes"].classbar and (Z.myclass == "PALADIN" or Z.myclass == "SHAMAN" or Z.myclass == "DRUID" or Z.myclass == "DEATHKNIGHT" or Z.myclass == "WARLOCK") then
		...
		if Z.myclass == "DRUID" then
			self:FontString("DruidMana", NUMFONT, FONTSIZE-1, "OUTLINEMONOCHROME")
	...
	if (unit and unit:find("arena%d")) or (unit and unit:find("boss%d")) then
		...
		if Z.isctm and (unit and unit:find('boss%d')) then
			...
			altpower:FontString("text", NUMFONTFONT, FONTSIZE-1, "OUTLINEMONOCHROME")
...
local function LoadDps()
  ...
  oUF:SetActiveStyle("Zod")  --3.35
	...
	tot:Point("BOTTOM", InvZuiActionBarBackground, "TOP", 0, 50)
	...
	--if C.unitframes.arena then
		...
		for i = 1, 5 do
			...
			if i == 1 then
				arena[i]:Point("BOTTOMLEFT", ZuiInfoRight, "TOPLEFT", -80, 305)
			else
	...
	--if C["unitframes"].showboss then
		...
		for i = 1, MAX_BOSS_FRAMES do
			...
			if i == 1 then
				boss[i]:Point("BOTTOMLEFT", ZuiInfoRight, "TOPLEFT", -80, 305)
			else
