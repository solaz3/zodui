...
-- this install Zui with default settings.
local function install()
	...
	SetCVar("cameraDistanceMaxFactor", 2.5)
  ...
  if Z.isctm then
	...
	if  C.chat.enable then					
		...
		for i = 1, NUM_CHAT_WINDOWS do
			...
			frame:Size(C.general.infowidth + 1, C.chat.height)
			
			-- tell wow that we are using new size
			SetChatWindowSavedDimensions(chatFrameId, Z.Scale(C.general.infowidth + 1), Z.Scale(C.chat.height))
			
			-- move general bottom left or Loot (if found) on right
			if i == 1 then
				frame:ClearAllPoints()
				frame:Point("BOTTOMLEFT", ZuiInfoLeft, "TOPLEFT", 0, 6)
			elseif i == 4 and chatName == "TRADE/LOOT" then
				frame:ClearAllPoints()
				frame:Point("BOTTOMRIGHT", ZuiInfoRight, "TOPRIGHT", 0, 6)
			end
	...
	-- tell savedvariable that we installed zui perfectly
	zDataPerChar.install = true
...
StaticPopupDialogs["ZODUIDISABLE_UI"] = {
...
StaticPopupDialogs["ZODUIINSTALL_UI"] = {
	text = L.popup_install,
	button1 = ACCEPT,
	button2 = CANCEL,
    OnAccept = install,
	OnCancel = function() zDataPerChar.install = true end,
    timeout = 0,
    whileDead = 1,
}

StaticPopupDialogs["ZODUIRESET_UI"] = {
	text = L.popup_reset,
	button1 = ACCEPT,
	button2 = CANCEL,
    OnAccept = install,
...
StaticPopupDialogs["ZODUIDISABLE_RAID"] = {
	text = L.popup_2raidactive,
	button1 = "DPS - TANK",
	button2 = "HEAL",
	OnAccept = function() DisableAddOn("zui_heal") EnableAddOn("zui_dps") ReloadUI() end,
	OnCancel = function() EnableAddOn("zui_heal") DisableAddOn("zui_dps") ReloadUI() end,
	timeout = 0,
	whileDead = 1,
}
...
ZuiOnLogon:SetScript("OnEvent", function(self, event)
	...
	if (zData == nil) then zData = {} end
	if (zDataPerChar == nil) then zDataPerChar = {} end

	if Z.getscreenwidth < 1200 then
    SetCVar("useUiScale", 0)
    StaticPopup_Show("ZODUIDISABLE_UI")
	else
		...
		if not zDataPerChar.install then			
			StaticPopup_Show("ZODUIINSTALL_UI")
		end
	end
	
	if (IsAddOnLoaded("zui_dps") and IsAddOnLoaded("zui_heal")) then
		StaticPopup_Show("ZODUIDISABLE_RAID")
	end
    -- nice inlog text yo	
	local name = Z.myname
...
SlashCmdList.CONFIGURE = function() StaticPopup_Show("ZODUIRESET_UI") end