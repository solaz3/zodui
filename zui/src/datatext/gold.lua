...
if C["datatext"].gold and C["datatext"].gold > 0 then
	...
	Z.SetDataText(C["datatext"].gold, Text)
	...
	local function OnEvent(self, event)
		...
		if (zData.gold == nil) then zData.gold = {}; end
		if (zData.gold[myPlayerRealm]==nil) then zData.gold[myPlayerRealm]={}; end
		zData.gold[myPlayerRealm][myPlayerName] = GetMoney();
	...
	Stat:SetScript("OnEnter", function(self)
		if not InCombatLockdown() then
			...
			local thisRealmList = zData.gold[myPlayerRealm];
