...
local ZuiSkin = CreateFrame("Frame")
ZuiSkin:RegisterEvent("ADDON_LOADED")
ZuiSkin:SetScript("OnEvent", function(self, event, addon)
	...
	if addon == "zui" then
		...
		for i = 1, getn(ChatMenus) do
			if _G[ChatMenus[i]] == _G["ChatMenu"] then
				_G[ChatMenus[i]]:HookScript("OnShow", function(self) self:SetTemplate("Default") self:ClearAllPoints() self:SetPoint("BOTTOMLEFT", ChatFrame1, "TOPLEFT", 0, Z.Scale(30)) end)
			else
		...
		for i = 1, getn(BlizzardMenuButtons) do
			local ZuiMenuButtons = _G["GameMenuButton"..BlizzardMenuButtons[i]]
			if ZuiMenuButtons then
				SkinButton(ZuiMenuButtons)
				if Z.isctm then
		...
		local ZuiButtons = _G[BlizzardButtons[i]]
			if ZuiButtons then
				SkinButton(ZuiButtons)
			end
		end
