...
local ZuiFonts = CreateFrame("Frame", nil, UIParent)
...
ZuiFonts:RegisterEvent("ADDON_LOADED")
ZuiFonts:SetScript("OnEvent", function(self, event, addon)
	if addon ~= "zui" then return end
	
	local NORMALPIXEL = C.media.pxfont
	local NORMAL     = C.media.font
	local COMBAT     = C.media.dmgfont
	local NUMBER     = C.media.pxfont
	...
	if Z.eyefinity then
		...
		local INVISIBLE = [=[Interface\Addons\zui\res\font\invisible_font.ttf]=]
		COMBAT = INVISIBLE
	end
	...
	SetFont(NumberFont_OutlineThick_Mono_Small, NUMBER, 12, "OUTLINEMONOCHROME")
	SetFont(NumberFont_Outline_Huge,            NUMBER, 28, "OUTLINEMONOCHROME", 28)
	SetFont(NumberFont_Outline_Large,           NUMBER, 12, "OUTLINEMONOCHROME")
	SetFont(NumberFont_Outline_Med,             NUMBER, 12, "OUTLINEMONOCHROME")
	SetFont(NumberFont_Shadow_Med,              NORMAL, 12, "OUTLINE")
	...
	SetFont(SystemFont_Outline_Small,           NUMBER, 12, "OUTLINEMONOCHROME")
	...
	SetFont(ZoneTextString,                     NORMAL, 15, "OUTLINE")
	SetFont(SubZoneTextString,                  NORMAL, 14, "OUTLINE")
	SetFont(PVPInfoTextString,                  NORMAL, 14, "OUTLINE")
	SetFont(PVPArenaTextString,                NORMAL, 14, "OUTLINE")
