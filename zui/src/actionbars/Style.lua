...
local function style(self)
	...
	if not _G[name.."Panel"] then
		-- resize all button not matching Z.buttonsize
		if self:GetHeight() ~= Z.buttonsize and not InCombatLockdown() then --Taint fix for Flyout Buttons
			self:SetSize(Z.buttonsize, Z.buttonsize)
		end

		-- create the bg/border panel
		local panel = CreateFrame("Frame", name.."Panel", self)
		panel:CreatePanel("Default", Z.buttonsize, Z.buttonsize, "CENTER", self, "CENTER", 0, 0)
	...
	HotKey:SetFont(C["media"].pxfont, 12, "OUTLINEMONOCHROME")
	HotKey.ClearAllPoints = Z.dummy
	HotKey.SetPoint = Z.dummy
...
local function stylesmallbutton(normal, button, icon, name, pet)
	...
	button.SetNormalTexture = Z.dummy
	
	Flash:SetTexture(media.buttonhover)
	
	if not _G[name.."Panel"] then
		button:SetWidth(Z.petbuttonsize)
		button:SetHeight(Z.petbuttonsize)
		
		local panel = CreateFrame("Frame", name.."Panel", button)
		panel:CreatePanel("Default", Z.petbuttonsize, Z.petbuttonsize, "CENTER", button, "CENTER", 0, 0)
		...
		icon:ClearAllPoints()
		if pet then			
			if Z.petbuttonsize < 30 then
				local autocast = _G[name.."AutoCastable"]
				autocast:SetAlpha(0)
			end
			local shine = _G[name.."Shine"]
			shine:Size(Z.petbuttonsize, Z.petbuttonsize)
			shine:ClearAllPoints()
			shine:SetPoint("CENTER", button, 0, 0)
			icon:Point("TOPLEFT", button, Z.Scale(2), Z.Scale(-2))
			icon:Point("BOTTOMRIGHT", button, Z.Scale(-2), Z.Scale(2))
		else
			icon:Point("TOPLEFT", button, 2, -2)
			icon:Point("BOTTOMRIGHT", button, -2, 2)
		end
...
local function updatehotkey(self, actionButtonType)
	...
	text = replace(text, '('..L.hotkey_mouse..')', 'M')
	text = replace(text, '('..L.hotkey_middle..')', 'M3')
	text = replace(text, '('..L.hotkey_scrolldown..')', 'MD')
	text = replace(text, '('..L.hotkey_scrollup..')', 'MU')
	text = replace(text, '('..L.hotkey_numpad..')', 'N')
	text = replace(text, '('..L.hotkey_pageup..')', 'PU')
	text = replace(text, '('..L.hotkey_pagedown..')', 'PD')
	text = replace(text, '('..L.hotkey_space..')', 'SpB')
	text = replace(text, '('..L.hotkey_insert..')', 'Ins')
	text = replace(text, '('..L.hotkey_home..')', 'Hm')
	text = replace(text, '('..L.hotkey_delete..')', 'Del')
...
if Z.isctm then hooksecurefunc("ActionButton_UpdateFlyout", styleflyout) end
...
if not Z.myclass == "SHAMAN" then return end
...
local function StyleTotemSlotButton(button, index)
	button:SetTemplate("Default")
	if Z.isctm then button.overlayTex:SetTexture(nil) end
	button.background:SetDrawLayer("ARTWORK")
	button.background:ClearAllPoints()
	button.background:SetPoint("TOPLEFT",button,"TOPLEFT",Z.Scale(2),Z.Scale(-2))
	button.background:SetPoint("BOTTOMRIGHT",button,"BOTTOMRIGHT",Z.Scale(-2),Z.Scale(2))
