...
anchor:SetAlpha(0)
if C.chat.background and ZuiChatBackgroundRight then
...
local function UpdateTooltip(self)
	...
	elseif self:GetAnchorType() == "ANCHOR_NONE" and InCombatLockdown() and C["tooltip"].hidecombat then
		self:Hide()
	end
...
hooksecurefunc("GameTooltip_SetDefaultAnchor", function(self, parent)
	if C["tooltip"].cursor then
		if IsAddOnLoaded("zui_heal") and parent ~= UIParent then
...
GameTooltipStatusBar:SetScript("OnValueChanged", function(self, value)
	...
	if not self.text then
		...
		self.text:SetFont(C["media"].pxfont, 12, "OUTLINEMONOCHROME")
		self.text:Show()
		if unit then
			min, max = UnitHealth(unit), UnitHealthMax(unit)
			local hp = Z.ShortValue(min).." / "..Z.ShortValue(max)
...
healthBar:SetStatusBarTexture(C.media.norm)
