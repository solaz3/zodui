...
local function SetTips(link, owner, tooltip1, tooltip2, tooltip3)
	...
	local left, right, anchor1, anchor2, xoff = owner:GetLeft(), owner:GetRight(), "TOPLEFT", "TOPRIGHT", 5
	if not left or not right then return end
	if (GetScreenWidth() - right) < left then 
	...
	if item1 then
		...
		tooltip1:SetPoint(anchor1, owner, anchor2, xoff, -10)
		tooltip1:Show()

		if item2 then
			tooltip2:ClearAllPoints()
			tooltip2:SetPoint(anchor1, tooltip1, anchor2, xoff, 0)
			tooltip2:Show()

			if item3 then
				tooltip3:ClearAllPoints()
				tooltip3:SetPoint(anchor1, tooltip2, anchor2, xoff, 0)
				tooltip3:Show()
			end
		end
	end
