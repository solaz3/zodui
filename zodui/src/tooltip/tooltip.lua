...
local ZuiTooltip = CreateFrame("Frame", "ZuiTooltip", UIParent)
...
local anchor = CreateFrame("Frame", "ZuiTooltipAnchor", UIParent)
anchor:SetSize(200, ZuiInfoRight:GetHeight())
...
-- we need a default point... -_-;
anchor:SetPoint("BOTTOMRIGHT", ZuiChatBackgroundRight, "TOPRIGHT", 0, 0)

anchor.text = Z.SetFontString(anchor, C.media.font, 10)
anchor.text:SetPoint("CENTER")
anchor.text:SetText(L.move_tooltip)

--[[ scripting this so tooltip moves depening how many bars we currently have up
local function AnchorTooltip(self)
	local db = ZuiDataPerChar
	local anchor = ZuiTooltipAnchor

 -- if db.hidebar5 == false then
			--anchor:SetPoint("BOTTOMRIGHT", ZuiChatBackgroundRight, "TOPRIGHT", 0, ZuiInfoRight:GetHeight() - (Z.buttonspacing * 2))
  --elseif db.hidebar5 == true then
			--anchor:SetPoint("BOTTOMRIGHT", ZuiChatBackgroundRight, "TOPRIGHT", 0, -ZuiInfoRight:GetHeight())
  --end

end

GameTooltip:HookScript("OnUpdate", function() AnchorTooltip(self) end)]]
...
local function UpdateTooltip(self)
	...
	if name and (ZuiPlayerBuffs or ZuiPlayerDebuffs) then
		if (ZuiPlayerBuffs:GetPoint():match("LEFT") or ZuiPlayerDebuffs:GetPoint():match("LEFT")) and (name:match("ZuiPlayerBuffs") or name:match("ZuiPlayerDebuffs")) then
			self:SetAnchorType("ANCHOR_BOTTOMRIGHT", x, -x)
		end
	end
		
	if (owner == MiniMapBattlefieldFrame or owner == MiniMapMailFrame) and ZuiMinimap then
		if ZuiMinimap:GetPoint():match("LEFT") then 
			self:SetAnchorType("ANCHOR_TOPRIGHT", x, -x)
		end
	end
	
	if self:GetAnchorType() == "ANCHOR_NONE" and ZuiTooltipAnchor then
		local point = ZuiTooltipAnchor:GetPoint()
		if point == "TOPLEFT" then
			self:ClearAllPoints()
			self:SetPoint("TOPLEFT", ZuiTooltipAnchor, "BOTTOMLEFT", 0, -x)			
		elseif point == "TOP" then
			self:ClearAllPoints()
			self:SetPoint("TOP", ZuiTooltipAnchor, "BOTTOM", 0, -x)			
		elseif point == "TOPRIGHT" then
			self:ClearAllPoints()
			self:SetPoint("TOPRIGHT", ZuiTooltipAnchor, "BOTTOMRIGHT", 0, -x)			
		elseif point == "BOTTOMLEFT" or point == "LEFT" then
			self:ClearAllPoints()
			self:SetPoint("BOTTOMLEFT", ZuiTooltipAnchor, "TOPLEFT", 0, x)		
		elseif point == "BOTTOMRIGHT" or point == "RIGHT" then
			if ZuiBags and ZuiBags:IsShown() then
				self:ClearAllPoints()
				self:SetPoint("BOTTOMRIGHT", ZuiBags, "TOPRIGHT", 0, x)			
			else
				self:ClearAllPoints()
				self:SetPoint("BOTTOMRIGHT", ZuiTooltipAnchor, "TOPRIGHT", 0, x)
			end
		else
			self:ClearAllPoints()
			self:SetPoint("BOTTOM", ZuiTooltipAnchor, "TOP", 0, x)		
		end
	end
end
...
hooksecurefunc("GameTooltip_SetDefaultAnchor", function(self, parent)
	if C["tooltip"].cursor == true then
		if IsAddOnLoaded("zodui_heal") and parent ~= UIParent then
...
GameTooltipStatusBar:SetScript("OnValueChanged", function(self, value)
	...
	if not self.text then
		...
		self.text:SetFont(font, fonts, fontf)
		self.text:Show()
		if unit then
...
local BorderColor = function(self)
	...
	elseif reaction then
		local c = Z.colors.reaction[reaction]
		r, g, b = c[1], c[2], c[3]
		self:SetBackdropBorderColor(r, g, b)
		healthBarBG:SetBackdropBorderColor(r, g, b)
		healthBar:SetStatusBarColor(r, g, b)
...
ZuiTooltip:RegisterEvent("PLAYER_ENTERING_WORLD")
ZuiTooltip:SetScript("OnEvent", function(self)
