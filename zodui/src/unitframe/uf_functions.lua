...
Z.LoadUFFunctions = function(layout)
	local oUF = zUF or oUF
	assert(oUF, "zUI was unable to locate oUF.")

	function Z.ContructHealthBar(self, bg, text)
    ...
    health.Smooth = true
		...
		if text then
			health:FontString("value", C["media"].numfont, C["unitframes"].fontsize*Z.raidscale, "THINOUTLINE")
		...
		health.backdrop:Point("TOPRIGHT", health, "TOPRIGHT", 2*Z.raidscale, 2*Z.raidscale)
		health.backdrop:Point("BOTTOMLEFT", health, "BOTTOMLEFT", -2*Z.raidscale, -2*Z.raidscale)
		health.backdrop:SetFrameLevel(health:GetFrameLevel() - 1)		
		
		return health
	end

	function Z.ConstructPowerBar(self, bg, text)
    ...
    power.Smooth = true
		...
		if text then
			power:FontString("value", C["media"].numfont, C["unitframes"].fontsize*Z.raidscale, "THINOUTLINE")
		...
		power.backdrop:Point("TOPRIGHT", power, "TOPRIGHT", 2*Z.raidscale, 2*Z.raidscale)
		power.backdrop:Point("BOTTOMLEFT", power, "BOTTOMLEFT", -2*Z.raidscale, -2*Z.raidscale)
		power.backdrop:SetFrameLevel(power:GetFrameLevel() - 1)
	
		return power
	end	
		
	function Z.ConstructCastBar(self, width, height, direction)
		local castbar = CreateFrame("StatusBar", nil, self)
		castbar:SetStatusBarTexture(C["media"].normTex)
		castbar:Height(height)
		castbar:Width(width - 4*Z.raidscale)
		...
		castbar.bg:Point("TOPLEFT", -2*Z.raidscale, 2*Z.raidscale)
		castbar.bg:Point("BOTTOMRIGHT", 2*Z.raidscale, -2*Z.raidscale)
		castbar.bg:SetFrameLevel(castbar:GetFrameLevel() - 1)
		
		castbar:FontString("Time", C["media"].numfont, C["unitframes"].fontsize*Z.raidscale, "THINOUTLINE")
		...
		castbar:FontString("Text", C["media"].uffont, C["unitframes"].fontsize*Z.raidscale, "THINOUTLINE")
		...
		if C["unitframes"].cbicons == true then
			...
			button:Height(height + 4*Z.raidscale)
			button:Width(height + 4*Z.raidscale)
			button:SetTemplate("Default")
			button:SetBackdropBorderColor(unpack(C["media"].bordercolor))
			if direction == "LEFT" then
				button:Point("RIGHT", castbar, "LEFT", -4*Z.raidscale, 0)
			else
				button:Point("LEFT", castbar, "RIGHT", 4*Z.raidscale, 0)
			end
			
			castbar.Icon = button:CreateTexture(nil, "ARTWORK")
			castbar.Icon:Point("TOPLEFT", button, 2*Z.raidscale, -2*Z.raidscale)
			castbar.Icon:Point("BOTTOMRIGHT", button, -2*Z.raidscale, 2*Z.raidscale)
	...
	local function CreateSwingStatusBar(parent, text)
		...
		if text then
			sbar:FontString("Text", C["media"].numfont, C["unitframes"].fontsize*Z.raidscale, "THINOUTLINE")
		...
		sbar.backdrop:Point("TOPLEFT", parent, "TOPLEFT", -2*Z.raidscale, 2*Z.raidscale)
		sbar.backdrop:Point("BOTTOMRIGHT", parent, "BOTTOMRIGHT", 2*Z.raidscale, -2*Z.raidscale)
		sbar.backdrop:SetTemplate("Default")
		return sbar
	end
	
	function Z.ConstructSwingBar(self, width, height, text)
		...
		swing.hideOoc = true
		swing:SetWidth(width*Z.raidscale)
		swing:SetHeight(height*Z.raidscale)
		return swing
	end
	...
	Z.PostUpdateHealth = function(health, unit, min, max)
		...
		if header == "ZuiHealParty" or header == "ZuiDPSParty" or header == "ZuiHealR6R25" or header == "ZuiDPSR6R25" or header == "ZuiHealR26R40" or header == "ZuiDPSR26R40" then --Raid/Party Layouts
          ...
          health.value:SetText("|cffD7BEA5D/C|r")
	...
	function Z.PostCreateAura(element, button)
		...
		button.icon:Point("TOPLEFT", 2*Z.raidscale, -2*Z.raidscale)
		button.icon:Point("BOTTOMRIGHT", -2*Z.raidscale, 2*Z.raidscale)
		button.icon:SetTexCoord(0.08, 0.92, 0.08, 0.92)
		button.icon:SetDrawLayer('ARTWORK')
		
		button.count:Point("BOTTOMRIGHT", Z.mult, 1.5)
		button.count:SetJustifyH("RIGHT")
		button.count:SetFont(C["media"].numfont, C["unitframes"].auratextscale*0.8, "THINOUTLINE")

		button.overlayFrame = CreateFrame("frame", nil, button, nil)
		button.cd:SetFrameLevel(button:GetFrameLevel() + 1)
		button.cd:ClearAllPoints()
		button.cd:Point("TOPLEFT", button, "TOPLEFT", 2*Z.raidscale, -2*Z.raidscale)
		button.cd:Point("BOTTOMRIGHT", button, "BOTTOMRIGHT", -2*Z.raidscale, 2*Z.raidscale)
	...
	function Z.createAuraWatch(self, unit)
		...
		if (buffs) then
			for key, spell in pairs(buffs) do
				...
				count:SetFont(C["media"].numfont, 8, "THINOUTLINE")
