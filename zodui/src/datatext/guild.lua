...
local displayString = string.join(Z.ValColor, "%s: ", "|cffFFFFFF", "%d|r")
...
local function Update(self, event, ...)	
	...
	if IsInGuild() then
		...
		Text:SetText(Z.ValColor..L.datatext_noguild)
		Stat:SetScript("OnMouseDown", nil)
	end
	self.update = false
end
	
local menuFrame = CreateFrame("Frame", "ZuiGuildRightClickMenu", UIParent, "UIDropDownMenuTemplate")
local menuList = {
...
Stat:SetScript("OnMouseUp", function(self, btn)
	...
	for i = 1, #guildTable do
		if guildTable[i][7] and guildTable[i][1] ~= Z.myname then
...
Stat:SetScript("OnEnter", function(self)
	...
	local anchor, panel, xoff, yoff = Z.DataTextTooltipAnchor(Text)
	GameTooltip:SetOwner(panel, anchor, xoff, yoff)
	GameTooltip:ClearLines()
	GameTooltip:AddDoubleLine(string.format(guildInfoString, GetGuildInfo('player')), string.format(guildInfoString2, L.datatext_guild, online, #guildTable),tthead.r,tthead.g,tthead.b,tthead.r,tthead.g,tthead.b)
	GameTooltip:AddLine(' ')
	...
	local col = Z.RGBToHex(ttsubh.r, ttsubh.g, ttsubh.b)
	GameTooltip:AddLine' '
	...
	if online > 1 then
		...
		for i = 1, #guildTable do
			...
			if connected and name ~= Z.myname then
