...
Text:SetShadowOffset(Z.mult, -Z.mult)
Z.PP(C["datatext"].friends, Text)

local menuFrame = CreateFrame("Frame", "ZuiFriendRightClickMenu", UIParent, "UIDropDownMenuTemplate")
local menuList = {
	...
	if BNTotalOnline > 0 then
		...
		for i = 1, #BNTable do
			if (BNTable[i][7]) then
				...
				if BNTable[i][6] == wowString and BNTable[i][11] == Z.myrealm and playerFaction == BNTable[i][12] then
