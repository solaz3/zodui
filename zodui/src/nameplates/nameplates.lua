...
local FONT = C["media"].numfont
local NFONT = C["media"].font
local FONTSIZE = 10
...
local tab = CLASS_FILTERS[Z.myclass].target
local function OnAura(frame, unit)
	...
	for index = 1,40 do
		...
		if C["nameplate"].trackccauras == true then
			if Z.DebuffWhiteList[name] then match = true end
		end
...
local function Colorize(frame)
	...
	for class, color in pairs(RAID_CLASS_COLORS) do
		...
		if RAID_CLASS_COLORS[class].r == r and RAID_CLASS_COLORS[class].g == g and RAID_CLASS_COLORS[class].b == b then
			...
			frame.hp:SetStatusBarColor(unpack(Z.colors.class[class]))
			return
		end
	end
	
	if g+b == 0 then -- hostile
		r,g,b = unpack(Z.colors.reaction[1])
		frame.isFriendly = false
	elseif r+b == 0 then -- friendly npc
		r,g,b = unpack(Z.colors.power["MANA"])
		frame.isFriendly = true
	elseif r+g > 1.95 then -- neutral
		r,g,b = unpack(Z.colors.reaction[4])
		frame.isFriendly = false
	elseif r+g == 0 then -- friendly player
		r,g,b = unpack(Z.colors.reaction[5])
		frame.isFriendly = true
	else -- enemy player
		frame.isFriendly = false
	end
...
local function SkinObjects(frame)
	...
	hp.name:SetFont(NFONT, FONTSIZE, FONTFLAG)
	hp.name:SetShadowColor(0, 0, 0, 0.4)
	hp.name:SetShadowOffset(Z.mult, -Z.mult)
	hp.oldname = oldname
	...
	cb.name:SetFont(NFONT, FONTSIZE, FONTFLAG)
...
local function CheckBlacklist(frame, ...)
	if Z.PlateBlacklist[frame.hp.name:GetText()] then
