-- Zui API, see DOCS/API.txt for more informations
...
Z.ValColor = ("|cff%.2x%.2x%.2x"):format(r * 255, g * 255, b * 255)
...
local function SetTemplate(f, t, tex)
	--if tex then texture = C.media.glowTex else texture = C.media.blank end
    ...
    --backdropr, backdropg, backdropb, backdropa = unpack(C["media"].fadedbackdropcolor)
		--tex = false
    backdropa = 0.8
	else 
  ...
  if tex and not f.texture then
    f:SetBackdropColor(0, 0, 0, backdropa)
		
		local texture = f:CreateTexture(nil, "BORDER")
		texture:Point("TOPLEFT", f, "TOPLEFT", 2, -2)
		texture:Point("BOTTOMRIGHT", f, "BOTTOMRIGHT", -2, 2)
		texture:SetTexture(C["media"].glossTex)
		texture:SetVertexColor(backdropr, backdropg, backdropb)
		texture:SetDrawLayer("BORDER", -8)
		f.texture = texture
	else
...
local function CreateShadow(f, t)
	...
	if t == "ClassColor" then
		local c = Z.oUF_colors.class[class]
		borderr, borderg, borderb = c[1], c[2], c[3]
		backdropr, backdropg, backdropb = unpack(C["media"].backdropcolor)
	end
	...
	shadow:SetBackdrop( { 
		edgeFile = C["media"].glowTex, edgeSize = Z.Scale(3),
