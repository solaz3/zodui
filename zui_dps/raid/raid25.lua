...
if C["raidframes"].griddps ~= true then
	...
	RAID_WIDTH = ((ZuiInfoLeft:GetWidth() / 5) - 2.5)*C["raidframes"].scale
...
local function Shared(self, unit)
  ...
  self:SetAttribute('initial-height', RAID_HEIGHT)
	...
	if C["raidframes"].griddps ~= true then
		...
		RaidDebuffs.count:SetFont(C["media"].pxfont, 12, "OUTLINEMONOCHROME")
		RaidDebuffs.count:SetPoint('BOTTOMRIGHT', RaidDebuffs, 'BOTTOMRIGHT', 0, 2)
		RaidDebuffs.count:SetTextColor(1, .9, 0)
		
		RaidDebuffs:FontString('time', C["media"].pxfont, 12, "OUTLINEMONOCHROME")
...
oUF:Factory(function(self)
	...
	if C["raidframes"].griddps ~= true then
		...
		raid:Point("BOTTOMLEFT", ZuiInfoLeft, "TOPLEFT", 1, 160)
	else
		raid = self:SpawnHeader("ZuiDPSR6R25", nil,"custom [@raid6,noexists][@raid26,exists] hide;show",
			'oUF-initialConfigFunction', [[
				local header = self:GetParent()
		...
		raid:Point("BOTTOMLEFT", ZuiInfoLeft, "TOPLEFT", 1, 155)	
	end
